...
// .wrangler/tmp/bundle-DAnLwl/checked-fetch.js
...
// node_modules/fountain-js/dist.esm/rules.js
var rules = {
...
// node_modules/fountain-js/dist.esm/utilities.js
...
// node_modules/fountain-js/dist.esm/token.js
var TitlePageBlock = class {
  static {
    __name(this, "TitlePageBlock");
  }
  constructor(source) {
    this.tokens = [];
...
var TitlePageToken = class {
  static {
    __name(this, "TitlePageToken");
  }
  constructor(pair) {
    this.is_title = true;
...
var SceneHeadingToken = class {
  static {
    __name(this, "SceneHeadingToken");
  }
  constructor(line) {
...
var CenteredToken = class {
  static {
    __name(this, "CenteredToken");
  }
  constructor(line) {
    this.type = "centered";
...
var TransitionToken = class {
  static {
    __name(this, "TransitionToken");
  }
  constructor(line) {
...
var DialogueBlock = class {
  static {
    __name(this, "DialogueBlock");
  }
...
${text}`;
      ...
      this.tokens.push(new CharacterToken(name.startsWith("@") ? name.replace(/^@/, "").trim() : name.trim()), new DialogueBeginToken(isDualDialogue ? "right" : dual ? "left" : void 0));
      if (dual) {
...
var DialogueBeginToken = class {
  static {
    __name(this, "DialogueBeginToken");
  }
  constructor(dual) {
...
var CharacterToken = class {
  static {
    __name(this, "CharacterToken");
  }
  constructor(text) {
    this.type = "character";
    this.text = text;
...
var DialogueToken = class {
  static {
    __name(this, "DialogueToken");
  }
  constructor(text) {
    this.type = "dialogue";
    this.text = text;
...
var DialogueEndToken = class {
  static {
    __name(this, "DialogueEndToken");
  }
  constructor() {
...
var ParentheticalToken = class {
  static {
    __name(this, "ParentheticalToken");
  }
  constructor(text) {
...
var DualDialogueBeginToken = class {
  static {
    __name(this, "DualDialogueBeginToken");
  }
  constructor() {
...
var DualDialogueEndToken = class {
  static {
    __name(this, "DualDialogueEndToken");
  }
  constructor() {
...
var LyricsToken = class {
  static {
    __name(this, "LyricsToken");
  }
  constructor(line) {
    this.type = "lyrics";
...
var SectionToken = class {
  static {
    __name(this, "SectionToken");
  }
  constructor(line) {
    this.type = "section";
...
var SynopsisToken = class {
  static {
    __name(this, "SynopsisToken");
  }
  constructor(line) {
    this.type = "synopsis";
...
var PageBreakToken = class {
  static {
    __name(this, "PageBreakToken");
  }
  constructor() {
...
var SpacesToken = class {
  static {
    __name(this, "SpacesToken");
  }
  constructor() {
    this.type = "spaces";
  }
...
var ActionToken = class {
  static {
    __name(this, "ActionToken");
  }
  constructor(line) {
    this.type = "action";
    this.text = line.replace(/^(\s*)!(?! )/gm, "$1").replace(/^( *)(\t+)/gm, (_, leading, tabs) => {
...
// node_modules/fountain-js/dist.esm/lexer.js
var Lexer = class _Lexer {
  ...
  static tokenize(script) {
    let source = script.replace(rules.boneyard, this.boneyardStripper).replace(/\r\n|\r/g, "\n");
    this.scanIndex = 0;
    const titlePageTokens = this.tokenizeTitlePage(source);
    source = source.substring(this.scanIndex);
    const scriptTokens = _Lexer.tokenizeScript(source);
  ...
  static tokenizeScript(source) {
    const lines = source.split(rules.blank_lines).reverse();
...
var InlineLexer = class {
  ...
  static reconstruct(line, escapeSpaces = false) {
    ...
    line = escapeHTML(
      line.replace(rules.escape, "[{{{$&}}}]")
      // perserve escaped characters
    );
    if (escapeSpaces) {
    ...
    return line.replace(rules.note_inline, this.inline.note).replace(/\n/g, this.inline.line_break).replace(/\[{{{\\(&.+?;|.)}}}]/g, this.inline.escape).trimEnd();
  }
};
InlineLexer.inline = {
  note: "<!-- $1 -->",
...
// node_modules/fountain-js/dist.esm/fountain.js
var Fountain = class {
  static {
    __name(this, "Fountain");
  }
  parse(script, getTokens) {
    ...
    try {
      ...
      if (titleToken === null || titleToken === void 0 ? void 0 : titleToken.text) {
        title = unEscapeHTML(InlineLexer.reconstruct(titleToken.text).replace("<br />", " ").replace(/<(?:.|\n)*?>/g, ""));
      }
      return {
        title,
        html: {
          title_page: titlePageTokens.map((token) => this.to_html(token)).join(""),
          script: scriptTokens.map((token) => this.to_html(token)).join("")
        },
  ...
  to_html(token) {
    ...
    if (token === null || token === void 0 ? void 0 : token.text) {
      lexedText = InlineLexer.reconstruct(token.text, token.type === "action");
    }
    switch (token.type) {
      case "title":
        return `<h1>${lexedText}</h1>`;
      case "author":
      case "authors":
        return `<p class="authors">${lexedText}</p>`;
      case "contact":
      case "copyright":
      case "credit":
      case "date":
      ...
      case "revision":
      case "source":
        return `<p class="${token.type.replace(/_/g, "-")}">${lexedText}</p>`;
      case "scene_heading":
        return `<h3${(token.scene_number ? ` id="${token.scene_number}">` : `>`) + lexedText}</h3>`;
      case "transition":
        return `<h2>${lexedText}</h2>`;
      case "dual_dialogue_begin":
        return `<div class="dual-dialogue">`;
      case "dialogue_begin":
        return `<div class="dialogue${token.dual ? " " + token.dual : ""}">`;
      case "character":
        return `<h4>${lexedText}</h4>`;
      case "parenthetical":
        return `<p class="parenthetical">${lexedText}</p>`;
      case "dialogue":
        return `<p>${lexedText}</p>`;
      case "dialogue_end":
        return `</div>`;
      ...
      case "action":
        return `<p>${lexedText}</p>`;
      case "centered":
        return `<p class="centered">${lexedText}</p>`;
      case "lyrics":
        return `<p class="lyrics">${lexedText}</p>`;
      case "page_break":
        return `<hr />`;
      case "spaces":
        return;
...
__name(generateAllThemesCSS, "generateAllThemesCSS");
...
function renderScreenplay(fountainText, currentEpisode, allEpisodes) {
...
__name(renderScreenplay, "renderScreenplay");
...
// .wrangler/tmp/bundle-DAnLwl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DAnLwl/middleware-loader.entry.ts
